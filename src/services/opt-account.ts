...
export class OperatorAccountService {
  ...
  static async GetOperatorAccount({ res, req, customData: userData }: PrivateMethodProps) {
    ...
    const pipLine: PipelineQuery = [
      { $match: { _id: new mongoose.Types.ObjectId(userData.auth_id) } },
    ...
    let checkUser: SendDBQuery = await UserOperatorModel.aggregate(pipLine).catch(e => ({ error: e }));
    ...
    return helpers.outputSuccess(res, checkUser);
  }
  ...
  static async DeleteOperatorUser({ body, id, res, req, customData: userData }: PrivateMethodProps) {
    ...
    const getData: SendDBQuery = await UserOperatorModel.findOne({ _id: userData.auth_id },
    ...
    if (!getData) {
      return helpers.outputError(res, null, "User account not found");
    }
    ...
    if (getData.account_type === "team") {
      removeData = await UserOperatorModel.findByIdAndDelete(userData.auth_id).catch(e => ({ error: e }));
    } else {
  ...
  //updating user data
  static async ChangeAdminPassword({ body, id, res, customData: userData }: PrivateMethodProps) {
    ...
    if (oldPass === newPass) {
      return helpers.outputError(res, null, "New password cannot be same as the old")
    }

    let checkUser: SendDBQuery = await UserOperatorModel.findById(userData.auth_id, null,
    ...
    //if the query does not execute
    if (!checkUser) return helpers.outputError(res, null, "Invalid account")

    //if the old password does not match
    if (!bcrypt.compareSync(oldPass, checkUser.password)) {
      return helpers.outputError(res, null, "Invalid Password")
    }
  ...
  static async CreateTeam({ body, id, req, res, customData: userData }: PrivateMethodProps) {
    let fullName = helpers.getInputValueString(body, "full_name")
    let email = helpers.getInputValueString(body, "email")
    let phoneNumber = helpers.getInputValueString(body, "phone_number")
    let roleID = helpers.getInputValueString(body, "role_id")

    let queryBuilder = {} as UserOperatorTypes

    if (userData.account_type !== "operator") {
      return helpers.outputError(res, null, "Only an operator can perform this action")
    }

    if (!id) {
      if (!fullName) return helpers.outputError(res, null, "Full name is required")
      //validate the email
      if (!email) return helpers.outputError(res, null, "email is required")
      //create the account
    ...
    if (fullName) {
      ...
      if (!/^[a-z\- ]+$/i.test(fullName)) {
        return helpers.outputError(res, null, "Full name should be alphabets only");
      }

      if (fullName.length < 4 || fullName.length > 60) {
        return helpers.outputError(res, null, fullName.length < 4 ? "Full name is too short" : "Full name is too long");
      }
      ...
      if (nameParts.length < 2) {
        return helpers.outputError(res, null, "Please provide at least your first and last name");
      }
      //if the name is more than 3
      if (nameParts.length > 3) {
        return helpers.outputError(res, null, "Not more than three names are allowed");
      }

      //if the name parts are less than 2 characters
      if (!helpers.hasAlphabet(nameParts[0], 2)) {
        return helpers.outputError(res, null, "Provided first name is not valid");
      }
      ...
      //if the name parts are less than 2 characters
      if (!helpers.hasAlphabet(nameParts[1], 2)) {
        return helpers.outputError(res, null, nameParts.length > 2 ? "The provided last name seems invalid" : "Your middle name doesn't look valid");
      }

      if (!helpers.hasAlphabet(nameParts[2], 2)) {
        return helpers.outputError(res, null, "The provided last name is not valid");
      }
    ...
    if (email) {
      ...
      if (!validator.isEmail(email)) return helpers.outputError(res, null, "Invalid email")

      //check if the email or phone exist
      let checkUser: SendDBQuery<UserOperatorTypes> = await UserOperatorModel.findOne({ email: email }, null,
        { lean: true }).catch(e => ({ error: e }))
      ...
      if (checkUser) return helpers.outputError(res, null, "Email already in use")
    ...
    if (roleID) {
      //if role is invalid
      if (helpers.isInvalidID(roleID)) {
        return helpers.outputError(res, null, "role_id is invalid")
      }
  ...
  static async UpdateTeamStatus({ body, id, res, customData: userData }: PrivateMethodProps) {
    let status = helpers.getInputValueString(body, "status")
    let suspendReason = helpers.getInputValueString(body, "suspend_reason")
    ...
    if (status === "2" && !suspendReason) return helpers.outputError(res, null, "Suspension reason is required")

    if (suspendReason) {
      //check the length
      if (suspendReason.length < 10 || suspendReason.length > 500) {
        return helpers.outputError(res, null, "Suspension reason must be between 10 and 500 characters")
      }
      //if it does not have text
      if (!helpers.hasAlphabet(suspendReason, 4)) {
        return helpers.outputError(res, null, "Suspension reason must have at least 4 alphabet characters")
      }
    }

    //get the current user data
    let getData: SendDBQuery = await UserOperatorModel.findOne({
      operator_id: new mongoose.Types.ObjectId(userData.auth_id),
      _id: new mongoose.Types.ObjectId(id), account_type: "team"
    ...
    if (getData.account_status === 0) {
      return helpers.outputError(res, null, "Cannot change status of a pending account")
    }
  ...
  static async GetTeam({ query, body, id, res, customData: userData }: PrivateMethodProps) {
    ...
    let qBuilder: ObjectPayload = {
      operator_id: new mongoose.Types.ObjectId(userData.operator_id),
      account_type: "team"
    } as UserOperatorTypes

    if (id) {
      if (helpers.isInvalidID(id)) {
        return helpers.outputError(res, null, 'Invalid auth ID')
      }
    ...
    let pipLine: PipelineQuery = [
      ...
      {
        $lookup: {
          from: DatabaseTableList.operator_roles,
          let: { roleID: "$role_id" },
          pipeline: [
    ...
    let getData: SendDBQuery = await UserOperatorModel.aggregate(pipLine).catch(e => ({ error: e }))
  ...
  static async DeleteTeam({ body, res, req, id, customData: userData }: PrivateMethodProps) {
    //if the user is not accoint holder
    if (userData.account_type !== "operator") {
      return helpers.outputError(res, null, "Only the admin can perform this action")
    }

    let deleteResult: SendDBQuery = await UserOperatorModel.findByIdAndDelete({
      operator_id: new mongoose.Types.ObjectId(userData.auth_id),
    ...
    if (!deleteResult) return helpers.outputError(res, null, "Account not found");
